...
const formChangeTemplate = (dot = {}) => {
  const { type, basePrice, destination, dateFrom, dateTo, offers } = dot;
  ...
  const getDestinationDate = () =>
    Destinations.find((x) => x.id === destination);
...
class FormChangeView {
  ...
  getTemplate() {
    return formChangeTemplate(this.dot);
  }
